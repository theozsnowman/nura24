...
class ForumModerationController extends Controller
{
    ...
    */
    ...
    {       
        ...
        $reports = DB::table('forum_reports')
            ...
            ->leftJoin('users', 'forum_reports.to_user_id', '=', 'users.id')
            ->select('forum_reports.*', 'forum_posts.content as post_content', 'users.name as reported_user_name', 'users.email as reported_user_email', 'users.avatar as reported_user_avatar', 
    ...
    */
    public function reports_topics_update (Request $request)
    {
        ...
        if ($request->has('close_topic')) $close_topic = 1; else $close_topic = 0;
        if ($request->has('delete_topic')) $delete_topic = 1; else $delete_topic = 0;
    ...
    */
    public function reports_posts_update (Request $request)
    {
        ...
        $inputs = $request->all(); // retrieve all of the input data as an array 
        if ($request->has('delete_post')) $delete_post = 1; else $delete_post = 0;
