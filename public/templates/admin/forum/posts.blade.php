<div class="card-header">
	<h3><i class="far fa-comment-alt"></i> {{ __('Forum posts') }} ({{ $posts->total() ?? 0 }})</h3>
</div>
<!-- end card-header -->
...
<div class="card-body">
	...
	<section>
			<form action="{{ route('admin.forum.posts') }}" method="get" class="form-inline">
				...
				<a class="btn btn-light" href="{{ route('admin.forum.posts') }}"><i class="fas fa-undo"></i></a>
			</form>
		</section>
		<div class="mb-3"></div>
					...
					<td>
						<div class="d-flex">
							<form method="POST" action="{{ route('admin.forum.posts.delete', ['id' => $post->id]) }}">
								{{ csrf_field() }}
								<button type="submit" class="float-right btn btn-danger btn-sm delete-item-{{$post->id}}"><i class="fas fa-trash-alt"></i></button>
							</form>
						</div>
